MODULE trcbc
   ...
   !!   'key_top'                                                TOP model 
   !!----------------------------------------------------------------------
   !!   trc_bc_init  : initialisation of passive tracer BC data
   !!   trc_bc_read  : read passive tracer boundary conditions data
   !!----------------------------------------------------------------------
...
CONTAINS
   ...
   SUBROUTINE trc_bc_init(ntrc)
      ...
      ! Initialize these arrays and structures so we avoid writing uninitialized garbage to output.namelist.top
      cn_dir_sbc(:) = ''
      cn_dir_obc(:) = ''
      cn_dir_cbc(:) = ''
      rn_trsfac(:)  = 0._wp
      ...
      DO jn = 1, jpmaxtrc
         sn_trcsbc(jn)%clname(:) = ''
         sn_trcsbc(jn)%nfreqh    = 0._wp
         sn_trcsbc(jn)%clvar(:)  = ''
         sn_trcsbc(jn)%ln_tint   = .FALSE.
         sn_trcsbc(jn)%ln_clim   = .FALSE.
         sn_trcsbc(jn)%cltype(:) = ''
         sn_trcsbc(jn)%wname(:)  = ''
         sn_trcsbc(jn)%vcomp(:)  = ''
         sn_trcsbc(jn)%lname(:)  = ''
         ...
         sn_trcobc(jn)%clname(:) = ''
         sn_trcobc(jn)%nfreqh    = 0._wp
         sn_trcobc(jn)%clvar(:)  = ''
         sn_trcobc(jn)%ln_tint   = .FALSE.
         sn_trcobc(jn)%ln_clim   = .FALSE.
         sn_trcobc(jn)%cltype(:) = ''
         sn_trcobc(jn)%wname(:)  = ''
         sn_trcobc(jn)%vcomp(:)  = ''
         sn_trcobc(jn)%lname(:)  = ''
         ...
         sn_trccbc(jn)%clname(:) = ''
         sn_trccbc(jn)%nfreqh    = 0._wp
         sn_trccbc(jn)%clvar(:)  = ''
         sn_trccbc(jn)%ln_tint   = .FALSE.
         sn_trccbc(jn)%ln_clim   = .FALSE.
         sn_trccbc(jn)%cltype(:) = ''
         sn_trccbc(jn)%wname(:)  = ''
         sn_trccbc(jn)%vcomp(:)  = ''
         sn_trccbc(jn)%lname(:)  = ''
...
906         IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy_bc in configuration namelist', lwp )
   ...
   !!----------------------------------------------------------------------
CONTAINS
   SUBROUTINE trc_bc_init( kt )        ! Empty routine
