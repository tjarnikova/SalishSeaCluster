MODULE trcnam_trp
   ...
   LOGICAL , PUBLIC ::   ln_trcadv_tvd_zts   ! TVD scheme flag with vertical sub time-stepping
   INTEGER , PUBLIC ::   nn_trcadv_tvd_zts   !    number of sub-time steps to use with TVD ZTS
...
CONTAINS
   ...
   SUBROUTINE trc_nam_trp
      ...
      NAMELIST/namtrc_adv/ ln_trcadv_cen2 , ln_trcadv_tvd   ,    &
         ...
         &                 ln_trcadv_msc_ups, ln_trcadv_tvd_zts, &
                           nn_trcadv_tvd_zts
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'namtrc_adv in configuration namelist', lwp )
      ...
      IF(lwp) THEN                    ! Namelist print
         ...
         WRITE(numout,*) '      TVD advection scheme with zts  ln_trcadv_tvd_zts = ', ln_trcadv_tvd_zts
         WRITE(numout,*) '      Number of sub time-steps for TVD ZTS  nn_trcadv_tvd_zts = ', nn_trcadv_tvd_zts
      ENDIF
      !
